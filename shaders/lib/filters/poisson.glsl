const vec2 poisson60o[60] = vec2[60] (
...
vec4 poisson12(sampler2D tex, float sigma) {
    vec2 pixelRad = sigma/vec2(viewWidth, viewHeight);
    vec4 col = vec4(0.0);

    for (int i = 0; i<12; i++) {
        vec2 bcoord = coord + poisson60o[i*(60/12)]*pixelRad;
        col += texture2D(tex, bcoord);
    }
    col /= 12;
    return col;
}
vec4 poisson20(sampler2D tex, float sigma) {
    vec2 pixelRad = sigma/vec2(viewWidth, viewHeight);
    vec4 col = vec4(0.0);

    for (int i = 0; i<20; i++) {
        vec2 bcoord = coord + poisson60o[i*(60/20)]*pixelRad;
        col += texture2D(tex, bcoord);
    }
    col /= 20;
    return col;

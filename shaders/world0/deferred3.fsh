...
void skyGradient() {
    ...
    float sunAlbedo = ismoothstep(sunGrad, 0.85, 0.95)*(1.0-timeMoon)*sunLimb;
    float moonAlbedo = ismoothstep(moonGrad, 0.85, 0.95)*timeNight;

    vec3 sunColor   = colSunglow;
    vec3 sunLight   = colSunlight*8;
    vec3 moonColor  = vec3(0.55, 0.75, 1.0)*0.2;

    vec3 albedoCol  = sunAlbedo*normalize(sunLight)*140.0 + moonAlbedo*normalize(moonColor)*6.0;
...
void simpleFog() {
    ...
    vec3 skyCol     = falloff>0.0 ? skyGradientF() : fogVanilla;
